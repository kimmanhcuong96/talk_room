...
export function YouTubeVideoStage({ video, canManage, language, onClose, onOwnerPlayback, onViewerPlayback }: {
  ...
  return <section className="theme-dark-surface relative h-full min-h-[200px] w-full overflow-hidden rounded-lg border border-white/10 bg-black shadow-2xl shadow-black/30">
...
export function YouTubeVideoDock({ videoId, language, onClick, compact = false, active = false }: { videoId: string; language: Language; onClick: () => void; compact?: boolean; active?: boolean }) {
  ...
  return <button type="button" onClick={onClick} title={label} aria-label={label} className={`theme-dark-surface group relative overflow-hidden rounded-lg border border-red-400/40 bg-black shadow-2xl shadow-black/40 transition hover:-translate-y-0.5 hover:border-red-400 ${compact ? "h-full w-full" : "aspect-video w-36 sm:w-44"}`}><img src={`https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`} alt="" referrerPolicy="no-referrer" className="h-full w-full object-cover opacity-80 transition group-hover:opacity-100"/><span className="absolute inset-0 grid place-items-center"><span className={`grid place-items-center rounded-xl bg-red-600 text-static-white shadow-lg ${compact ? "h-8 w-10" : "h-10 w-12"}`}><Play size={compact ? 16 : 20} fill="currentColor"/></span></span></button>;
}
