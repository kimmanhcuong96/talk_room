...
import { getTopicBackground, getTopicBackgroundImage, getTopicFont, getTopicIcon, topicBackgrounds, topicFonts, topicIcons } from "../lib/roomTopic";
...
export function RoomTopicSlide({ topic, language, compact = false, fill = false }: { topic: RoomTopicValue; language: Language; compact?: boolean; fill?: boolean }) {
  ...
  const backgroundImage = getTopicBackgroundImage(topic.background);
  return <section aria-label={roomTopicTranslate(language, "topic")} style={backgroundImage ? { backgroundImage: `linear-gradient(rgba(2, 6, 23, .25), rgba(2, 6, 23, .25)), url(${backgroundImage})`, backgroundSize: "cover", backgroundPosition: "center" } : undefined} className={`theme-dark-surface relative isolate flex w-full shrink-0 items-center justify-center overflow-hidden rounded-2xl border px-6 text-center shadow-xl shadow-black/20 ${compact ? "min-h-40 py-6" : fill ? "h-full min-h-0 py-8 sm:px-12" : "h-[clamp(10rem,32dvh,20rem)] py-8 sm:px-12"} ${getTopicBackground(topic.background)}`}>
    ...
    <div className="max-h-full w-full max-w-4xl overflow-y-auto">
      {Icon ? <span className={`mx-auto grid shrink-0 place-items-center rounded-2xl border border-white/15 bg-white/10 text-white shadow-lg shadow-black/10 ${compact ? "h-11 w-11" : "h-14 w-14 sm:h-16 sm:w-16"}`}><Icon size={compact ? 22 : 29}/></span> : null}
      <p className={`mx-auto whitespace-pre-wrap break-words text-white/95 ${Icon ? "mt-4" : ""} ${compact ? "text-sm leading-6" : "text-base leading-7 sm:text-xl sm:leading-8 lg:text-2xl"} ${getTopicFont(topic.font)}`}>{topic.description}</p>
    </div>
  </section>;
}
...
export function RoomTopicEditor({ topic, language, error, saving, onClose, onSave }: { topic: RoomTopicValue | null; language: Language; error: string | null; saving: boolean; onClose: () => void; onSave: (topic: RoomTopicValue | null) => void }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/70 px-4 py-6 backdrop-blur-sm" onMouseDown={onClose}><form onSubmit={(event) => { event.preventDefault(); const clean = description.trim(); if (!clean) { setLocalError(t("required")); return; } onSave({ description: clean, background, font, icon }); }} onMouseDown={(event) => event.stopPropagation()} className="w-full max-w-lg rounded-xl border border-white/10 bg-panel p-5 text-white shadow-2xl">
    ...
    <fieldset className="mt-4"><legend className="text-sm text-white/70">{t("icon")}</legend><div className="mt-2 flex flex-wrap gap-2">{topicIcons.map(({ value, Icon }) => <button key={value} type="button" title={value === "none" ? `${t("icon")}: —` : value} aria-label={value === "none" ? `${t("icon")}: —` : value} aria-pressed={icon === value} onClick={() => setIcon(value)} className={`grid h-10 w-10 place-items-center rounded-md border ${icon === value ? "border-mint bg-mint/10 text-mint" : "border-white/10 text-white/55 hover:bg-white/5"}`}><Icon size={18}/></button>)}</div></fieldset>
