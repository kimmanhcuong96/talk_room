...
import { getTurnUsage, getWebRtcUsage, type TurnUsageStatus, type WebRtcUsage } from "../../lib/adminAuth";
...
export function UsageAnalyticsPage({ token, language }: { token: string; language: Language }) {
  ...
  useEffect(() => {
    let cancelled = false;
    const load = () => { setRefreshing(true); return Promise.allSettled([getWebRtcUsage(token), getTurnUsage(token)]).then(([webrtc, turn]) => {
      if (cancelled) return;
      const failures = [webrtc, turn].filter((result): result is PromiseRejectedResult => result.status === "rejected");
  ...
  return <div className="grid gap-5">
    ...
    <section className="grid gap-4 md:grid-cols-4">{(["daily","weekly","monthly","yearly"] as const).map((period) => <div key={period} className="rounded-lg border border-white/10 bg-panel p-4"><p className="text-xs uppercase tracking-wide text-white/45">{copy[period]}</p><p className="mt-2 text-lg font-semibold"><strong>{copy.stun} {duration(usage?.[period].stun.seconds ?? 0)}</strong></p><p className="text-xs text-white/55">{usage?.[period].stun.connections ?? 0} {copy.connections ?? "connections"}</p><p className="mt-2 text-sm text-[#55aaff]">{copy.turn} {duration(usage?.[period].turn.seconds ?? 0)}</p><p className="text-xs text-white/55">{usage?.[period].turn.connections ?? 0} {copy.connections ?? "connections"}</p></div>)}</section>
    <section className="rounded-lg border border-white/10 bg-panel p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><h3 className="font-semibold">{copy.turnBandwidth}</h3><p className="mt-1 text-sm text-white/55">{copy.turnBandwidthDescription}</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${turnUsage?.turnAllowed ? "bg-mint/15 text-mint" : "bg-coral/15 text-coral"}`}>{turnUsage?.configured ? (turnUsage.turnAllowed ? copy.turnAvailable : copy.turnLimitReached) : copy.cloudflareNotConfigured}</span></div><div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><div><p className="text-xs uppercase tracking-wide text-white/45">{copy.bytes}</p><p className="mt-1 text-xl font-semibold">{turnUsage?.egressBytes == null ? "—" : turnUsage.egressBytes.toLocaleString()}</p></div><div><p className="text-xs uppercase tracking-wide text-white/45">{copy.mb}</p><p className="mt-1 text-xl font-semibold text-[#55aaff]">{turnUsage?.egressMb == null ? "—" : turnUsage.egressMb.toFixed(2)}</p></div><div><p className="text-xs uppercase tracking-wide text-white/45">{copy.gb}</p><p className="mt-1 text-xl font-semibold text-mint">{turnUsage?.egressGb == null ? "—" : turnUsage.egressGb.toFixed(4)}</p></div><div><p className="text-xs uppercase tracking-wide text-white/45">{copy.maxTurnUsage}</p><p className="mt-1 text-xl font-semibold text-amber-200">{turnUsage?.limitGb == null ? "—" : `${turnUsage.limitGb} GB`}</p></div></div><p className="mt-3 text-xs text-white/40">{copy.lastCloudflareCheck}: {turnUsage?.checkedAt ? new Date(turnUsage.checkedAt).toLocaleString() : "—"}</p></section>
    <section className="overflow-x-auto rounded-lg border border-white/10 bg-panel"><h3 className="border-b border-white/10 px-4 py-3 font-semibold">{copy.dailyHistory}</h3><table className="w-full min-w-[680px] text-left text-sm"><thead className="border-b border-white/10 text-xs uppercase tracking-wide text-white/45"><tr><th className="px-4 py-3">{copy.date}</th><th className="px-4 py-3">{copy.stun}</th><th className="px-4 py-3">STUN connections</th><th className="px-4 py-3">{copy.turn}</th><th className="px-4 py-3">TURN connections</th></tr></thead><tbody className="divide-y divide-white/8">{[...new Set((usage?.series ?? []).map((row) => row.date))].slice(-31).reverse().map((date) => { const stun = (usage?.series ?? []).find((row) => row.date === date && row.transport === "stun"); const turn = (usage?.series ?? []).find((row) => row.date === date && row.transport === "turn"); return <tr key={date}><td className="px-4 py-3">{date}</td><td className="px-4 py-3">{duration(stun?.seconds ?? 0)}</td><td className="px-4 py-3">{stun?.connections ?? 0}</td><td className="px-4 py-3">{duration(turn?.seconds ?? 0)}</td><td className="px-4 py-3">{turn?.connections ?? 0}</td></tr>; })}</tbody></table></section>
  </div>;
}
