  ...
  return <div className="grid gap-5">
    ...
    {editing ? <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-black/70 p-4 backdrop-blur-sm" onMouseDown={() => setEditing(null)}><section className="my-auto w-full max-w-2xl rounded-xl border border-white/10 bg-panel p-5 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-sm text-white/65">Single sentence probability (%)<input type="number" min={0} max={100} step={1} value={editing.singleSentenceProbability} onChange={(e) => setEditing({ ...editing, singleSentenceProbability: Number(e.target.value), twoSentenceProbability: 100 - Number(e.target.value) })} className="mt-1 h-10 w-full rounded-md border border-white/10 bg-field px-3 text-white"/></label>
        <label className="text-sm text-white/65">Two sentence probability (%)<input type="number" min={0} max={100} step={1} value={editing.twoSentenceProbability} onChange={(e) => setEditing({ ...editing, twoSentenceProbability: Number(e.target.value), singleSentenceProbability: 100 - Number(e.target.value) })} className="mt-1 h-10 w-full rounded-md border border-white/10 bg-field px-3 text-white"/></label>
