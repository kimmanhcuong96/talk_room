...
import { VerificationRequestsPageBulk } from "./VerificationRequestsPageBulk";
...
function AdminLayout({ session, title, language, t, onSignOut, children }: {
    ...
    <div>
      ...
      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => {
          ...
          const content = <><div className="flex items-start justify-between"><span className="grid h-12 w-12 place-items-center rounded-lg bg-[#258ff4]/15 text-[#55aaff]"><Icon size={24} /></span>{!card.allowed ? <span className="rounded-full bg-reward/15 px-3 py-1 text-xs font-semibold text-reward">{t("ownerOnly")}</span> : null}</div><h2 className="mt-5 text-xl font-semibold">{card.title}</h2><p className="mt-2 leading-6 text-white/60">{card.description}</p><span className="mt-6 inline-flex items-center gap-2 font-semibold text-mint">{card.allowed ? t("manage") : t("accessDenied")}</span></>;
          return card.allowed
  ...
  return (
    <div className="grid gap-5">
      ...
      <form className="flex flex-col gap-3 rounded-lg border border-white/10 bg-panel p-4 sm:flex-row" onSubmit={(event) => { event.preventDefault(); setPage(1); setSearch(searchInput.trim()); }}>
        ...
        <button className="h-10 rounded-md bg-[#258ff4] px-5 text-sm font-semibold text-static-white hover:bg-[#1d7edb]">{t("search")}</button>
      </form>
      ...
      {rewardOverview ? <section className="grid gap-3 rounded-lg border border-white/10 bg-panel p-4 lg:grid-cols-3">
        <div><p className="text-xs uppercase tracking-wide text-white/45">{t("pointsToday")}</p><strong className="mt-1 block text-2xl text-reward">{rewardOverview.daily[rewardOverview.daily.length - 1]?.points ?? 0}</strong></div>
...
export function AdminApp() {
  ...
  if (page === "verification-requests") content = <VerificationRequestsPageBulk token={session.token} language={language} t={t} />;
  ...
  return <AdminLayout session={session} title={pageTitle} language={language} t={t} onSignOut={signOut}>{content}</AdminLayout>;
}
