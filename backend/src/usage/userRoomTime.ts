...
export function startUserRoomSession(socketId: string, userId: string | undefined, roomId: string) {
  if (!userId) return;
  const now = Date.now();
  active.set(socketId, { userId, roomId, startedAt: now });
}

function getEligibleSegments(session: ActiveSession, endedAt: number) {
  return [{ startedAt: session.startedAt, endedAt }];
}
...
export async function finishUserRoomSession(socketId: string) {
  ...
  try {
    ...
    try {
      ...
      const rewardSegments = getEligibleSegments(session, endedAt);
